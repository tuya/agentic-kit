...
- **`AGENTIC_KIT_LOG_LEVEL` is the one compile-time log gate, SDK-wide.** Every SDK log macro
  (log_tag_* in `common/log.h`; iot-client's log_error family, TAI_LOG*, TUYA_BLE_HAL_LOG*
  re-tagged on top) expands to nothing above it. `log_emit()` remains the public sink (handler
  plumbing, runtime filter), but module code never calls it directly — a raw call would bypass
  the ceiling. The one site with a runtime-chosen level, the media sampler in tai_pkt_log.c,
  dispatches over the gated macros by its two possible runtime levels. The old per-module
  `TAI_LOG_LEVEL` gate is gone.

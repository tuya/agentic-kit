...
> **Migration**: the old per-module `-DTAI_LOG_LEVEL=N` has been merged into `-DAGENTIC_KIT_LOG_LEVEL=N` (it applies to the **entire SDK**, not just rtc-tcp-client). Code that called `log_set_level(N)` at boot: drop the call, compile with `-DAGENTIC_KIT_LOG_LEVEL=N` instead, or filter by level inside your `AGENTIC_KIT_LOG` target. Code that installed a handler with `log_set_handler()`: make that function the `AGENTIC_KIT_LOG` target — it now receives the level and the bare tag directly, no need to strip the tag prefix out of the format string anymore.

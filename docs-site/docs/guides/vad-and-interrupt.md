...
## 云端 VAD vs 设备端 VAD：SDK 调用对比 {#云端-vad-vs-设备端-vadsdk-调用对比}
...
### 何时使用 `tai_send_audio_end` {#何时使用-tai_send_audio_end}
...
### 手动按键 / 设备端 VAD 模式 {#手动按键--设备端-vad-模式}

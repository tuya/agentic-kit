...
:::warning 最常见的错误
在云端 VAD 模式下，收到 `TAI_EVT_SERVER_VAD` 后调用 `tai_send_audio_end()` 是**错误**的——它会主动结束当前上行 Event，导致云端截断用户语音、连续对话中断。云端 VAD 模式下整个聊天会话只调用一次 `tai_send_audio_start()`，且**不能**调用 `tai_send_audio_end()`。
:::
...
| 收到 `TAI_EVT_SERVER_VAD` | 仅更新本地状态（结束"聆听中"提示、切换 UI）；**不动上行流** | 通常不依赖此事件（本地已自行判停） |
...
:::warning
云端 VAD 模式下，收到 `TAI_EVT_SERVER_VAD` **不要**调用 `tai_send_audio_end()`。该函数会结束上行 Event 并通知云端"输入完毕"，在连续对话中调用它会导致云端截断用户语音。参考实现见 xiaozhi-esp32（`tuya_protocol.cc` 中 `TAI_EVT_SERVER_VAD` 仅向应用层上报本地状态变化）。
:::
...
|------|--------------------------------|
| 云端 VAD 连续对话（收到 `TAI_EVT_SERVER_VAD`） | **否**，上行流保持打开 |
...
- 收到 `TAI_EVT_SERVER_VAD` 后**不要**调用 `tai_send_audio_end()`（云端 VAD 模式下整个会话都不调用）
- 云端 VAD 模式下 `TAI_EVT_CHAT_BREAK` 的处理只需清理下行播放，上行音频流保持打开

...
### 2. 把智能体投放到产品（设备直连） {#2-把智能体投放到产品设备直连}
...
### 2. 规则命中的是"变化"，不是"状态" {#2-规则命中的是变化不是状态}
...
### 5. 触发器也可能要求智能体控制设备 {#5-触发器也可能要求智能体控制设备}
...
### 6. 上报类型不硬编码，从 schema 解析 {#6-上报类型不硬编码从-schema-解析}

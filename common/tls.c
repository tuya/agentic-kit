    ...
    if (n == MBEDTLS_ERR_SSL_WANT_WRITE || n == MBEDTLS_ERR_SSL_WANT_READ) {
        if (tls_request_expired(t, req)) {
            ...
            log_emit(LOG_ERROR,
                     ...
                     (unsigned)req->timeout_ms, (unsigned)req->written,
                     (unsigned)req->len);
            return TLS_ERR_NET;
        }
...
static int tls_drive_read(tls_t *t, tls_io_req_t *req)
{
    ...
    while (n == MBEDTLS_ERR_SSL_RECEIVED_NEW_SESSION_TICKET)
        n = mbedtls_ssl_read(&t->ssl, req->read_buf, req->len);
#endif
    if (n > 0) return n;
